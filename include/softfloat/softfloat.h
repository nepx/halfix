...
// To work around some C99 limitations
#define packFloatx80Constant(sign, exponent, mantissa) { .exp = (sign << 15) | exponent, .fraction = mantissa }
#define packFloat128Constant(high, low) { .hi = high, .lo = low }

#define floatx80_unpack(st, expdest, sigdest) do { expdest = (st)->exp; sigdest = (st)->fraction; }while(0)
#define floatx80_repack(st, expdest, sigdest) do { (st)->exp = expdest; (st)->fraction = sigdest; }while(0)

// Halfix-specific defines
#define BUILD_FLOAT(e, s) { .exp = e, .fraction = s}
